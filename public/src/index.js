async function getData() {
    ...
    try {
            ...
            if (array_base[i] != array_comp[j]) {
                ...
                if (!similar) {
                    if (isSimilar(array_base[i + 1], array_comp[j], threshold)) {
                        ...
                        console.log(array_base[i], array_comp[j + 1], isSimilar(array_base[i], array_comp[j + 1], threshold))
        ...
        console.log(e)
        if (arrayMenor) {
            text_check += `<span class="green-missed">${array_base.slice(j).join(' ')}</span>`
